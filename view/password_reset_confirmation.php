...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email'])) {
    ...
    if ($user) {
        ...
        // Ajoute le token à la base de données
        if ($userCon->addToken($email, $token)) {
            ...
            try {
                // Paramètres SMTP
                $mail->isSMTP();
                $mail->Host = 'smtpauths.bluewin.ch';
                $mail->SMTPAuth = true;
                $mail->Username = 'hase.tritten@bluewin.ch';
                $mail->Password = 'Leitwolf1';
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
                $mail->Port = 465;

                // Destinataire et contenu de l'e-mail
                $mail->setFrom('hase.tritten@bluewin.ch', 'voyage visit');
            ...
            echo "Failed to add token to the database. Please try again later.";
        }
    } else {
          ...
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-center py-5 px-4 px-sm-5">
              ...
              <h1>Password Reset Link Sent</h1>
                ...
                <p>Please check your email inbox and follow the instructions provided.</p>
            </div>
          </div>
        </div>
      </div>
